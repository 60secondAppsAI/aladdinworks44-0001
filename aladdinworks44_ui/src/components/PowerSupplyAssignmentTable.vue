        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Power Supply Assignments</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalPowerSupplyAssignments = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPowerSupplyAssignments">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add PowerSupplyAssignment</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PowerSupplyAssignmentId" type="text" placeholder="Enter PowerSupplyAssignmentId" v-model="powerSupplyAssignmentToAdd.powerSupplyAssignmentId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="powerSupplyAssignments" :row-key="record => record.PowerSupplyAssignmentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PowerSupplyAssignmentPictureView :powerSupplyAssignments="powerSupplyAssignments" />
          </div>
...
import PowerSupplyAssignmentPictureView from './PowerSupplyAssignmentPictureView.vue';


const powerSupplyAssignmentsColumns = [
  "powerSupplyAssignmentId",
  "year",
  "date",
  "competitionId",
  "powerSupplyAssignmentId"
...
export default {
  ...
  components: {
    ...
    Pagination,
    PowerSupplyAssignmentPictureView
  },

  data() {
    return {
      modalPowerSupplyAssignments: false,
        isTableView: true,

      columns: [
        {
          title: 'Power Supply Assignment Id',
		dataIndex: 'powerSupplyAssignmentId',
          visible: true,
          scopedSlots: { customRender: 'powerSupplyAssignmentId' },
          sorter: true
          //sorter: (a, b) => a.powerSupplyAssignmentId - b.powerSupplyAssignmentId,
          //sorter: (a, b) => a.powerSupplyAssignmentId.localeCompare(b.powerSupplyAssignmentId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} powerSupplyAssignments`,
      },

      powerSupplyAssignments: [],
      powerSupplyAssignmentToAdd: {},

      powerSupplyAssignmentsTable: {
        columns: [...powerSupplyAssignmentsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'powerSupplyAssignmentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPowerSupplyAssignmentsTable() {
      ...
      let powerSupplyAssignmentsTableData = [];
      for (let i = 0; i < this.powerSupplyAssignments.length; i++) {
        powerSupplyAssignmentsTableData.push({
          id: i,
          powerSupplyAssignmentId: this.powerSupplyAssignments[i].powerSupplyAssignmentId,
          year: this.powerSupplyAssignments[i].year,
          date: this.powerSupplyAssignments[i].date,
          competitionId: this.powerSupplyAssignments[i].competitionId,
          powerSupplyAssignmentId: this.powerSupplyAssignments[i].powerSupplyAssignmentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-power-supply-assignments',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPowerSupplyAssignmentsChanged', this.searchQuery);
		//this.renderPowerSupplyAssignmentsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalPowerSupplyAssignments = false;

      const currentDate = new Date().getTime();
      this.powerSupplyAssignmentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.powerSupplyAssignmentToAdd);
      console.log(jsonData);
      
      const res = await PowerSupplyAssignmentService.addPowerSupplyAssignment(jsonData);
