        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Rack Assignments</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalRackAssignments = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalRackAssignments">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add RackAssignment</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="RackAssignmentId" type="text" placeholder="Enter RackAssignmentId" v-model="rackAssignmentToAdd.rackAssignmentId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="rackAssignments" :row-key="record => record.RackAssignmentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <RackAssignmentPictureView :rackAssignments="rackAssignments" />
          </div>
...
import RackAssignmentPictureView from './RackAssignmentPictureView.vue';


const rackAssignmentsColumns = [
  "rackAssignmentId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    RackAssignmentPictureView
  },

  data() {
    return {
      modalRackAssignments: false,
        isTableView: true,

      columns: [
        {
          title: 'Rack Assignment Id',
		dataIndex: 'rackAssignmentId',
          visible: true,
          scopedSlots: { customRender: 'rackAssignmentId' },
          sorter: true
          //sorter: (a, b) => a.rackAssignmentId - b.rackAssignmentId,
          //sorter: (a, b) => a.rackAssignmentId.localeCompare(b.rackAssignmentId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} rackAssignments`,
      },

      rackAssignments: [],
      rackAssignmentToAdd: {},

      rackAssignmentsTable: {
        columns: [...rackAssignmentsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'rackAssignmentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderRackAssignmentsTable() {
      ...
      let rackAssignmentsTableData = [];
      for (let i = 0; i < this.rackAssignments.length; i++) {
        rackAssignmentsTableData.push({
          id: i,
          rackAssignmentId: this.rackAssignments[i].rackAssignmentId,
          year: this.rackAssignments[i].year,
          date: this.rackAssignments[i].date,
          competitionId: this.rackAssignments[i].competitionId,
          rackAssignmentId: this.rackAssignments[i].rackAssignmentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-rack-assignments',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForRackAssignmentsChanged', this.searchQuery);
		//this.renderRackAssignmentsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalRackAssignments = false;

      const currentDate = new Date().getTime();
      this.rackAssignmentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.rackAssignmentToAdd);
      console.log(jsonData);
      
      const res = await RackAssignmentService.addRackAssignment(jsonData);
