        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalMaintenanceSchedules = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalMaintenanceSchedules">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add MaintenanceSchedule</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="MaintenanceScheduleId" type="text" placeholder="Enter MaintenanceScheduleId" v-model="maintenanceScheduleToAdd.maintenanceScheduleId"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="maintenanceScheduleToAdd.description"></base-input>
  <base-input label="Date" type="text" placeholder="Enter Date" v-model="maintenanceScheduleToAdd.date"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="maintenanceSchedules" :row-key="record => record.MaintenanceScheduleId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <MaintenanceSchedulePictureView :maintenanceSchedules="maintenanceSchedules" />
          </div>
...
import MaintenanceSchedulePictureView from './MaintenanceSchedulePictureView.vue';


const maintenanceSchedulesColumns = [
...
export default {
  ...
  components: {
    ...
    Pagination,
    MaintenanceSchedulePictureView
  },

  data() {
    return {
      modalMaintenanceSchedules: false,
        isTableView: true,

      columns: [
        {
          title: 'Maintenance Schedule Id',
		dataIndex: 'maintenanceScheduleId',
          visible: true,
          scopedSlots: { customRender: 'maintenanceScheduleId' },
          sorter: true
          //sorter: (a, b) => a.maintenanceScheduleId - b.maintenanceScheduleId,
          //sorter: (a, b) => a.maintenanceScheduleId.localeCompare(b.maintenanceScheduleId),
        },
        {
          title: 'Description',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} maintenanceSchedules`,
      },

      maintenanceSchedules: [],
      maintenanceScheduleToAdd: {},
      ...
      sortBy: 'maintenanceScheduleId',           // Column to sort by
  ...
  methods: {
    ...
    async renderMaintenanceSchedulesTable() {
      ...
      let maintenanceSchedulesTableData = [];
      for (let i = 0; i < this.maintenanceSchedules.length; i++) {
        maintenanceSchedulesTableData.push({
          id: i,
          maintenanceScheduleId: this.maintenanceSchedules[i].maintenanceScheduleId,
          year: this.maintenanceSchedules[i].year,
          date: this.maintenanceSchedules[i].date,
          competitionId: this.maintenanceSchedules[i].competitionId,
          maintenanceScheduleId: this.maintenanceSchedules[i].maintenanceScheduleId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-maintenance-schedules',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForMaintenanceSchedulesChanged', this.searchQuery);
		//this.renderMaintenanceSchedulesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalMaintenanceSchedules = false;

      const currentDate = new Date().getTime();
      this.maintenanceScheduleToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.maintenanceScheduleToAdd);
      console.log(jsonData);
      
      const res = await MaintenanceScheduleService.addMaintenanceSchedule(jsonData);
