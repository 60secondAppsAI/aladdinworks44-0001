...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalServers = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalServers">
                <template slot="header">
  ...
  <base-input label="CpuLoad" type="text" placeholder="Enter CpuLoad" v-model="serverToAdd.cpuLoad"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="servers" :row-key="record => record.ServerId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ServerPictureView :servers="servers" />
          </div>
...
import ServerPictureView from './ServerPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'serverId',
          visible: true,
          scopedSlots: { customRender: 'serverId' },
          sorter: true
		...
		dataIndex: 'cpuLoad',
          visible: true,
          scopedSlots: { customRender: 'cpuLoad' },
          sorter: true
          //sorter: (a, b) => a.cpuLoad - b.cpuLoad,
          //sorter: (a, b) => a.cpuLoad.localeCompare(b.cpuLoad),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'serverId',           // Column to sort by
  ...
  methods: {
    ...
    async renderServersTable() {
      ...
      let serversTableData = [];
      for (let i = 0; i < this.servers.length; i++) {
        serversTableData.push({
          id: i,
          serverId: this.servers[i].serverId,
          year: this.servers[i].year,
          date: this.servers[i].date,
          competitionId: this.servers[i].competitionId,
          serverId: this.servers[i].serverId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-servers',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForServersChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.serverToAdd.created = currentDate;
