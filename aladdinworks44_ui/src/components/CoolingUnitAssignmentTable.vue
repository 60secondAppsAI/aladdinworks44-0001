        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Cooling Unit Assignments</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalCoolingUnitAssignments = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCoolingUnitAssignments">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add CoolingUnitAssignment</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="CoolingUnitAssignmentId" type="text" placeholder="Enter CoolingUnitAssignmentId" v-model="coolingUnitAssignmentToAdd.coolingUnitAssignmentId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="coolingUnitAssignments" :row-key="record => record.CoolingUnitAssignmentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <CoolingUnitAssignmentPictureView :coolingUnitAssignments="coolingUnitAssignments" />
          </div>
...
import CoolingUnitAssignmentPictureView from './CoolingUnitAssignmentPictureView.vue';


const coolingUnitAssignmentsColumns = [
  "coolingUnitAssignmentId",
  "year",
  "date",
  "competitionId",
  "coolingUnitAssignmentId"
...
export default {
  ...
  components: {
    ...
    Pagination,
    CoolingUnitAssignmentPictureView
  },

  data() {
    return {
      modalCoolingUnitAssignments: false,
        isTableView: true,

      columns: [
        {
          title: 'Cooling Unit Assignment Id',
		dataIndex: 'coolingUnitAssignmentId',
          visible: true,
          scopedSlots: { customRender: 'coolingUnitAssignmentId' },
          sorter: true
          //sorter: (a, b) => a.coolingUnitAssignmentId - b.coolingUnitAssignmentId,
          //sorter: (a, b) => a.coolingUnitAssignmentId.localeCompare(b.coolingUnitAssignmentId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} coolingUnitAssignments`,
      },

      coolingUnitAssignments: [],
      coolingUnitAssignmentToAdd: {},

      coolingUnitAssignmentsTable: {
        columns: [...coolingUnitAssignmentsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'coolingUnitAssignmentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCoolingUnitAssignmentsTable() {
      ...
      let coolingUnitAssignmentsTableData = [];
      for (let i = 0; i < this.coolingUnitAssignments.length; i++) {
        coolingUnitAssignmentsTableData.push({
          id: i,
          coolingUnitAssignmentId: this.coolingUnitAssignments[i].coolingUnitAssignmentId,
          year: this.coolingUnitAssignments[i].year,
          date: this.coolingUnitAssignments[i].date,
          competitionId: this.coolingUnitAssignments[i].competitionId,
          coolingUnitAssignmentId: this.coolingUnitAssignments[i].coolingUnitAssignmentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-cooling-unit-assignments',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCoolingUnitAssignmentsChanged', this.searchQuery);
		//this.renderCoolingUnitAssignmentsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalCoolingUnitAssignments = false;

      const currentDate = new Date().getTime();
      this.coolingUnitAssignmentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.coolingUnitAssignmentToAdd);
      console.log(jsonData);
      
      const res = await CoolingUnitAssignmentService.addCoolingUnitAssignment(jsonData);
