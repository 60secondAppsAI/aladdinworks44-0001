        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Network Switchs</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalNetworkSwitchs = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalNetworkSwitchs">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add NetworkSwitch</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="NetworkSwitchId" type="text" placeholder="Enter NetworkSwitchId" v-model="networkSwitchToAdd.networkSwitchId"></base-input>
  <base-input label="Model" type="text" placeholder="Enter Model" v-model="networkSwitchToAdd.model"></base-input>
  <base-input label="NumberOfPorts" type="text" placeholder="Enter NumberOfPorts" v-model="networkSwitchToAdd.numberOfPorts"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="networkSwitchs" :row-key="record => record.NetworkSwitchId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <NetworkSwitchPictureView :networkSwitchs="networkSwitchs" />
          </div>
...
import NetworkSwitchPictureView from './NetworkSwitchPictureView.vue';


const networkSwitchsColumns = [
  "networkSwitchId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    NetworkSwitchPictureView
  },

  data() {
    return {
      modalNetworkSwitchs: false,
        isTableView: true,

      columns: [
        {
          title: 'Network Switch Id',
		dataIndex: 'networkSwitchId',
          visible: true,
          scopedSlots: { customRender: 'networkSwitchId' },
          sorter: true
          //sorter: (a, b) => a.networkSwitchId - b.networkSwitchId,
          //sorter: (a, b) => a.networkSwitchId.localeCompare(b.networkSwitchId),
        },
        {
          title: 'Model',
		dataIndex: 'model',
		...
		dataIndex: 'numberOfPorts',
          visible: true,
          scopedSlots: { customRender: 'numberOfPorts' },
          sorter: true
          //sorter: (a, b) => a.numberOfPorts - b.numberOfPorts,
          //sorter: (a, b) => a.numberOfPorts.localeCompare(b.numberOfPorts),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} networkSwitchs`,
      },
      ...
      networkSwitchsTable: {
        columns: [...networkSwitchsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'networkSwitchId',           // Column to sort by
  ...
  methods: {
    ...
    async renderNetworkSwitchsTable() {
      ...
      let networkSwitchsTableData = [];
      for (let i = 0; i < this.networkSwitchs.length; i++) {
        networkSwitchsTableData.push({
          id: i,
          networkSwitchId: this.networkSwitchs[i].networkSwitchId,
          year: this.networkSwitchs[i].year,
          date: this.networkSwitchs[i].date,
          competitionId: this.networkSwitchs[i].competitionId,
          networkSwitchId: this.networkSwitchs[i].networkSwitchId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-network-switchs',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForNetworkSwitchsChanged', this.searchQuery);
		//this.renderNetworkSwitchsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalNetworkSwitchs = false;

      const currentDate = new Date().getTime();
      this.networkSwitchToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.networkSwitchToAdd);
      console.log(jsonData);
      
      const res = await NetworkSwitchService.addNetworkSwitch(jsonData);
