        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalAssignments = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalAssignments">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Assignment</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="AssignmentId" type="text" placeholder="Enter AssignmentId" v-model="assignmentToAdd.assignmentId"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="assignmentToAdd.description"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="assignments" :row-key="record => record.AssignmentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <AssignmentPictureView :assignments="assignments" />
          </div>
...
import AssignmentPictureView from './AssignmentPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    AssignmentPictureView
  },

  data() {
    return {
      modalAssignments: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'assignmentId',
          visible: true,
          scopedSlots: { customRender: 'assignmentId' },
          sorter: true
          //sorter: (a, b) => a.assignmentId - b.assignmentId,
          //sorter: (a, b) => a.assignmentId.localeCompare(b.assignmentId),
        },
        {
          title: 'Description',
      ...
      sortBy: 'assignmentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderAssignmentsTable() {
      ...
      let assignmentsTableData = [];
      for (let i = 0; i < this.assignments.length; i++) {
        assignmentsTableData.push({
          id: i,
          assignmentId: this.assignments[i].assignmentId,
          year: this.assignments[i].year,
          date: this.assignments[i].date,
          competitionId: this.assignments[i].competitionId,
          assignmentId: this.assignments[i].assignmentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-assignments',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForAssignmentsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalAssignments = false;

      const currentDate = new Date().getTime();
      this.assignmentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.assignmentToAdd);
      console.log(jsonData);
      
      const res = await AssignmentService.addAssignment(jsonData);
